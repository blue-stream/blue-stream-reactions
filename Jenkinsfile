...
""") {
  ...
  node(label) {
    ...
    stage('run a blue-stream-reaction test') {
      container('jenkins-build-slave') {
        withCredentials([string(credentialsId: 'ACRUSER', variable: 'ACRUSER'), string(credentialsId: 'ACRPASS', variable: 'ACRPASS'), string(credentialsId: 'BS_RMQ_SERVER', variable: 'RMQ_SERVER'), string(credentialsId: 'BS_DB_SERVER', variable: 'DB_SERVER')]) {
          ...
          sh "docker build -t bluehub.azurecr.io/blue-stream/blue-stream-reaction:${GIT_SHORT_COMMIT} ."
          sh "docker run --env DB_SERVER=$DB_SERVER --env RMQ_HOST=$RMQ_SERVER bluehub.azurecr.io/blue-stream/blue-stream-reaction:${GIT_SHORT_COMMIT} npm test"       
        }
      }
    }
  }
